...
## 4b. Default ที่ตั้งไว้แล้ว (inert — ปรับได้ทุกเมื่อ)
เพื่อเดินหน้า foundation โดยไม่ต้องรอ ผมตั้งค่า default ต่อไปนี้ (ยังไม่ charge เงิน):
- **dedup window = 24 ชม.** (Q1) — ยิงซ้ำเจ้าเดิม×spec เดิมใน 1 วัน = lead เดียว
- **ยังไม่ cap fan-out** (Q1) — คงพฤติกรรม RFQ เดิม ไม่แตะ flow ที่ทำงานอยู่
- **`spec_sync` = analytics เท่านั้น ไม่เก็บเงินผู้ขาย** (Q2 ยืนยันตามคำแนะนำ)
- **quote_request event** ถูกบันทึกแล้วตอนส่ง RFQ (ledger, creditCost=0) รอเปิด billing
> ค่าจริงของราคา (Q6) + PDPA (Q3) ยังเป็น 🛑 ที่ต้องคน/ทนาย — ยังเปิด charge ไม่ได้
